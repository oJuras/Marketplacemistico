            ...
            <tbody>
                ${sellerProducts.map(product => `
                    <tr>
                        ...
                        <td>R$ ${parseFloat(product.preco).toFixed(2).replace('.', ',')}</td>
...
function renderCart() {
    ...
    const subtotal = shoppingCart.reduce((sum, item) => sum + (parseFloat(item.preco) * item.quantidade), 0);
    ...
    container.innerHTML = shoppingCart.map(item => `
        <div class="cart-item">
            <div class="cart-item-info">
                ...
                <p class="product-price">R$ ${parseFloat(item.preco).toFixed(2).replace('.', ',')}</p>
            </div>
